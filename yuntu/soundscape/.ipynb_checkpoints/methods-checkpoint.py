...
def soundscapeSetLoadNodes(sc):
    ...
    sc.setNode("eCols",["e_"+str(i) for i in range(sc.config["energyParams"]["fBins"])])
    sc.setNode("eTransform",loadTransform(sc.config["energyParams"]["energyTransform"]))
    sc.setNode("sTransform",loadTransform(sc.config["energyParams"]["signalTransform"]))
    ...
    sc.setNode("splits",(scOps.makeSplits,"fragment","basicMeta","splitMeta","groupMeta","eCols","sTransform","eTransform","energyParams"),True)
